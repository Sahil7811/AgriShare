...
exports.getProducts = async (req, res) => {
  try {
    const { username, type, location, minPrice, maxPrice, condition } =
      req.query;
...
exports.getSalesMetrics = async (req, res) => {
  try {
    ...
    const totalProductsForRent = await Product.countDocuments({
      username: username,
      available: true,
    });
    ...
    const averageReviews =
      totalProductsForRent > 0 ? totalReviews / totalProductsForRent : 0;
