...
const ProductDetail = () => {
  ...
  const fetchProduct = useCallback(async () => {
    ...
    try {
      ...
      const userReviewCount = productData.reviews.filter(
      ...
      <Row className="mb-4">
        ...
        <Col md={6}>
          <Card className="product-details-card shadow-sm">
            <Card.Body>
              <Card.Title>
                {product.name || t("productDetail.defaultName")}
              </Card.Title>
              ...
              <Card.Text>
                <small className="text-muted">
                  {t("productDetail.location")}:{" "}
                  {product.location || t("productDetail.notSpecified")}
                </small>
              </Card.Text>
              <Card.Text>
                <Badge bg="info" className="badge">
                  {product.type || t("productDetail.typeNotSpecified")}
                </Badge>
              </Card.Text>
              <Card.Text>
                <strong>
                  {t("productDetail.rentalDuration")}:{" "}
                  {product.rentalDuration || t("productDetail.notSpecified")}
                </strong>
              </Card.Text>
              <Card.Text>
                <strong>
              ...
              <Button
                variant="primary"
                className="w-100 bttn"
                onClick={handleRentNow}
                disabled={!product.available || isOwner}
              >
        ...
        <Col>
          <Card className="shadow-sm reviews">
            <Card.Body>
              ...
              <p>
                {averageRating.toFixed(1)} {t("productDetail.stars")}
              </p>
              <Card.Title>
                ...
                {displayedReviews.length > 0 ? (
                  displayedReviews.map((review, index) => (
                    <ListGroup.Item key={index}>
                      ...
                      <StarRating rating={review.rating} size={24} readonly />
                      <p>{review.comment}</p>
                    </ListGroup.Item>
                  ))
                ) : (
